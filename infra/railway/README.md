...
- `web` : Next.js, futur domaine `braincrew.ai` ;
...
Railway ne permet pas encore de créer les domaines personnalisés depuis l’IaC. Après l’application du plan, ajouter `braincrew.ai` au service `web` et `api.braincrew.ai` au service `api` dans **Settings → Networking**, puis configurer les DNS indiqués.
